...
Your loop:
1. Use list_directory and search_code to discover legacy PHP files.
2. Use read_file to inspect each one.
3. Identify security issues (SQL injection, hardcoded credentials, missing input validation,
...
5. When all legacy files have a modernized counterpart, produce a final markdown report
...
export async function runModernizationWorkflow({ provider, onEvent } = {}) {
  ...
  try {
    ...
    return await agent.run(prompt, { onEvent });
  } finally {
    await mcp.close();
  }
}
